...
void ComputeHErr(const Real4* theta, int n_theta, Real* err_out, int64_t n_errs, int n_joints)
{
#if defined _GPU_PARALLEL
	...
	for (int64_t i_err = 0; i_err < n_errs; i_err ++)
	{
		...
		for (int i_joint = 0; i_joint < n_joints; i_joint ++)
		{
			auto q_i = theta[i_theta_q_base + i_joint];
			auto q_j = theta[j_theta_q_base + i_joint];
